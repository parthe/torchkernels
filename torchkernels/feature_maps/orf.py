...
class ORF:
	def __init__(self, 
		...
		if shape_matrix is not None:
			self.shape_matrix = shape_matrix.to(self.dtype)
		...
		del Q_arr
		self.Q = torch.from_numpy(self.Q).to(self.dtype).to(self.device)
		...
		self.set_S()
		if self.bias_term:
			self._bias = ((torch.rand(self._num_features, dtype=self.dtype) * math.pi * 2).to(self.device))
