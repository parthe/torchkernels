...
feature_map = MaternORF(input_dim=d, num_features=p, nu=1.5, shape_matrix=M)
...
if torch.allclose(feature_map(X), Phi, atol=1e-4):
    print("Matern ORF test complete")
    
feature_map = MaternRFF(input_dim=d, num_features=p, nu=1.5, shape_matrix=M)
DEVICE = feature_map.device
X = torch.randn(n,d).to(DEVICE)
...
if torch.allclose(feature_map(X), Phi, atol=1e-4):
    print("Matern RFF test complete")